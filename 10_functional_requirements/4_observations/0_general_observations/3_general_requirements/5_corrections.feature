...
Feature: Corrections and Deletions
  ...
  Then the <User> can supply a <Corrected Measurement>
   ...
   And the <Infrastructure> can report that <Corrected Measurement>
  ...
  Then the <User> can mark the <Measurement> as deleted or invalid
   ...
   And the <Infrastructure> will either no longer report the incorrect <Measurement> or will report it as being invalid.
